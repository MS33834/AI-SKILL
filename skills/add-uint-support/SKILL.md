...
description: 'Add unsigned integer (uint) type support to PyTorch operators by updating
  ...
  operators, kernels, or when user mentions e...'
description_zh:
category: dev-tools
tags:
  - ai
...
Use this skill when adding uint16, uint32, or uint64 support to PyTorch operators, when the user mentions "unsigned types", "uint support", or "barebones unsigned types", or when enabling support for kUInt16, kUInt32, kUInt64 in kernels and dispatch macros.
...
Adding uint support to a PyTorch CUDA operator:

**Before** (operator only supports signed types):
```cpp
// aten/src/ATen/native/cuda/ReduceOpsKernel.cu
void min_values_kernel_cuda(TensorIterator& iter) {
  AT_DISPATCH_V2(iter.dtype(), "min_values_cuda", AT_WRAP([&]() {
    gpu_reduce_kernel<scalar_t, scalar_t>(
      iter, [](scalar_t* out, scalar_t* inp, int n, scalar_t init) {
        return min_impl<scalar_t>().reduce(inp, n, init);
      }, at::numeric_limits<scalar_t>::max());
  }), AT_EXPAND(AT_ALL_TYPES), kHalf, kBFloat16);
}
```

**After** (uint16, uint32, uint64 support added):
```cpp
// aten/src/ATen/native/cuda/ReduceOpsKernel.cu
void min_values_kernel_cuda(TensorIterator& iter) {
  AT_DISPATCH_V2(iter.dtype(), "min_values_cuda", AT_WRAP([&]() {
  ...
  //                           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  //                           Added: kUInt16, kUInt32, kUInt64 support
}
```
...
# Create tensors with unsigned integer types
x_uint16 = torch.tensor([5, 3, 8, 1], dtype=torch.uint16, device='cuda')
x_uint32 = torch.tensor([100, 200, 50], dtype=torch.uint32, device='cuda')
x_uint64 = torch.tensor([1000, 2000, 500], dtype=torch.uint64, device='cuda')

# These now work (previously would raise RuntimeError)
print(torch.min(x_uint16))  # tensor(1, dtype=torch.uint16)
print(torch.min(x_uint32))  # tensor(50, dtype=torch.uint32)
print(torch.min(x_uint64))  # tensor(500, dtype=torch.uint64)
```
