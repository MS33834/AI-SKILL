...
name: drizzle
name_zh: drizzle
description: LobeHub Drizzle ORM schema and query style. Use for pgTable 
  schemas, indexes, joins, inferred types, db.select/db.query, schema fields, 
  foreign keys, junction tables, or postgres query patterns.
description_zh: LobeHub Drizzle ORM schema and query style. Use for pgTable 
  schemas, indexes, joins, inferred types, db.select/db.query, schema fields, 
  foreign keys, junction tables, or postgres query patterns.
category: dev-tools
tags:
...
Use this skill when you need guidance on drizzle.
...
Complete example demonstrating schema definition, type inference, and query patterns:

```typescript
// packages/database/src/schemas/posts.ts
import { pgTable, text, varchar, boolean, jsonb, uniqueIndex } from 'drizzle-orm/pg-core';
import { eq, asc, count } from 'drizzle-orm';
import { idGenerator, timestamps } from './_helpers';
import { users } from './users';

/**
 * Blog posts with draft/published status and metadata
 */
...
// Repository pattern with proper query style
export class PostRepository {
  constructor(private db: Database) {}

  async findById(id: string): Promise<Post | undefined> {
    // ✅ Use db.select() builder, not relational API
...
const authorPostCount = await postRepo.countByAuthor(userId);
```
