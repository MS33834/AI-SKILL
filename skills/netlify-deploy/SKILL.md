...
description_zh: 部署 web projects to Netlify using the Netlify CLI (`npx 
  netlify`). Use when the 用户 asks to 部署, host, publish, or link a site/repo on 
  Netlify, including preview and production deploys.
category: dev-tools
tags:
  - ai
  - api
...
Use this skill when you need guidance on netlify-deploy.
...
Complete deployment workflow for a Next.js application:

```bash
#!/bin/bash
# deploy.sh - Automated Netlify deployment script

set -e

# Step 1: Verify authentication
echo "Checking Netlify authentication..."
if ! npx netlify status | grep -q "Logged in"; then
  echo "Not authenticated. Please run: npx netlify login"
  exit 1
fi

# Step 2: Check if site is linked
SITE_LINKED=$(npx netlify status | grep -c "Site ID" || true)
...
if [ "$SITE_LINKED" -eq 0 ]; then
  ...
  if [ -n "$REMOTE_URL" ]; then
    ...
    npx netlify link --git-remote-url "$REMOTE_URL" || {
      echo "Site not found on Netlify. Creating new site..."
      npx netlify init --yes
    }
  else
    echo "No Git remote found. Creating new site..."
    npx netlify init --yes
  fi
fi
...
# Step 5: Deploy to preview (for testing)
echo "Deploying to preview..."
PREVIEW_URL=$(npx netlify deploy --json | jq -r '.deploy_url')
...
```toml
# netlify.toml - Configuration for Next.js project
[build]
...
```typescript
// netlify/functions/api.ts - Netlify Function example
...
// package.json - Deployment scripts
{
  "scripts": {
    ...
    "deploy:functions": "netlify deploy --prod --functions",
