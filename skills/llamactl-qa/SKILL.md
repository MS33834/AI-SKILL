...
User request or task description related to llamactl QA testing.

# Output

Generated content based on the user request, including QA reports and test results.
...
Complete QA run for a change to `deployments get` output formatting:

**Planning phase** - Draft matrix for review:

| # | Command | Backend | Look for | Covers |
|---|---|---|---|---|
| 1 | `deployments get my-app -o yaml --project test-proj` | test-env | spec/status split reads cleanly; field ordering; null vs omitted | new display model shape |
| 2 | `deployments get --project test-proj` | test-env | column choices, alignment, behavior with long repo URLs | plain-table list mode |
| 3 | `deployments get my-app -o json --project test-proj` | test-env | JSON structure matches API contract; no unexpected nulls | JSON output mode |
| 4 | `deployments get my-app --project test-proj` vs `deployments get my-app` | test-env | does the `--project` override actually retarget | project override |

**Execution** - Run row 1 and capture output:

```bash
$ llamactl deployments get my-app -o yaml --project test-proj
```
...
status:
  state: Running
  endpoint: https://api.example.com/my-app
  warning: null
  last_updated: "2026-06-12T10:30:00Z"
```

Notes:
- `spec:` and `status:` split is clean, but `warning: null` feels noisy when there's no warning
- Field ordering looks logical (user-facing fields first)
- `suspended: false` on a fresh deployment - is this worth showing or just noise?

**Report** - Write to `thoughts/shared/qa/2026-06-12-llamactl-deployments-get.md`:

```markdown
# llamactl QA: deployments get output formatting

Backend: test-env
Profile: qa-profile (https://test-api.example.com)
Project: test-proj
Branch: feature/new-output-format  Commit: abc1234

## Design questions for the reviewer

- Is `spec:` + `status:` the right split, or is the extra indent annoying?
- `warning: null` shows explicitly - should we omit nulls or keep them for completeness?
- `suspended: false` on fresh deployments - noise or useful state indicator?

## Rows

### 1. deployments get my-app -o yaml
...
- Consider omitting null fields from status block
- Document when `suspended` field is meaningful
```
