...
description: Audit an Anthropic Cookbook notebook based on a rubric. Use 
  whenever a notebook review or audit is requested.
description_zh: Audit an Anthropic Cookbook notebook based on a rubric. Use 
  whenever a notebook 审查 or audit is requested.
category: dev-tools
tags:
  - ai
  - api
...
Use this skill when you need guidance on cookbook-audit.
...
python3 validate_notebook.py path/to/cookbook.ipynb

# This generates a markdown report in tmp/ folder and runs detect-secrets
# to scan for hardcoded API keys and credentials
```

```python
# Example audit scoring workflow
from style_guide import load_rubric

# Load the style guide and rubrics
rubric = load_rubric("style_guide.md")

# Score each dimension
narrative_score = score_narrative_quality(notebook, rubric)
code_score = score_code_quality(notebook, rubric)
technical_score = score_technical_accuracy(notebook, rubric)
actionability_score = score_actionability(notebook, rubric)

# Generate audit report
total_score = narrative_score + code_score + technical_score + actionability_score
