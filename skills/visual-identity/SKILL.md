...
description: Build and maintain a persistent visual identity for your agent 
  using Flux Kontext Pro. Use when the user asks the agent to generate selfies, 
  avatars, character art, or any image that should look like the same person 
  across generations.
description_zh: 构建 and maintain a persistent visual identity for your 智能体 using 
  Flux Kontext Pro. Use when the 用户 asks the 智能体 to 生成 selfies, avatars, 
  character art, or any image that should look like the same person across 
  generations.
category: dev-tools
tags:
  - ai
...
```bash
# Generate a base character from text
python3 <path-to-skill>/scripts/generate_image.py generate \
  --prompt "A portrait of a young woman with shoulder-length auburn hair, green eyes, light freckles, wearing a black leather jacket. Clean background, studio lighting, 3:4 portrait." \
  --out /tmp/base_character.jpg

# Save as reference
mkdir -p ~/.letta/agents/$AGENT_ID/reference/visual-identity
cp /tmp/base_character.jpg ~/.letta/agents/$AGENT_ID/reference/visual-identity/canonical.jpg

# Generate new scene with same identity
python3 <path-to-skill>/scripts/generate_image.py edit \
  --reference ~/.letta/agents/$AGENT_ID/reference/visual-identity/canonical.jpg \
  --prompt "The same person is sitting at a desk coding late at night, lit by monitor glow. Keep her exact face, bone structure, eye color, and hair." \
  --out /tmp/coding_scene.jpg
```
