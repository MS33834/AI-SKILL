...
本仓库已从"技能保险柜"转型为**开源社区驱动的 AI 技能仓库索引**。

### 本地精选技能（38 条）

转型后保留 38 个有真实内容的 `SKILL.md`，分三类：

#### 手写 vendor-neutral（5 条，无 `source:` 字段）
...
所有本地技能的 license 已统一为 MIT。

## 外部仓库索引（928 条）

本仓库的**主要功能**是提供 928+ 个 AI 技能仓库的可搜索索引。
索引数据在 `external-index/skills.yaml`，每条记录包含：

- `source` / `source_url` — 上游仓库地址（我们只索引，不分发）
- `domain` — 9 大功能领域之一
- `vendor_type` — 大厂 / 热门社区 / 社区 / 个人
- `category` — 49 个子分类之一
- `skills` — 该仓库提供的具体技能列表
- `stars` / `license` / `pushed_at` — 元数据（由 `sync-github.py` 刷新）

**索引不等于分发**。我们只链接到上游仓库，不复制其内容。用户点
"Visit on GitHub ↗" 直达源头。每个仓库的协议看各自卡片上的
`license` 字段。
...
外部索引中的仓库均为公开 GitHub 仓库，我们只做索引和分类，不
复制内容。如果仓库所有者不希望被索引，开 issue 即可移除。
